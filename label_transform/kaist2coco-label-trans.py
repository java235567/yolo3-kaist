...
kaist_label_path = '/dataset/annotations/'

# transformed lables to save path
kaist_label_tosave_path = '/dataset/data/kaist/labels/'
kaist_list_tosave_path = '/dataset/data/kaist/'
...
'''
kaist_names = open('/dataset/data/kaist.names', mode='r') #only person
...
for phase in ['train', 'test']:  # train_set, test_set
    ...
    if phase == 'train':
        allset = train_set
        f = open('{2}{1}_{0}.txt'.format(flag, phase, kaist_list_tosave_path),
                 'w')  # create list of images for training
    elif phase == 'test':
        allset = test_set
        f = open('{2}val_{0}.txt'.format(flag, phase, kaist_list_tosave_path),
                 'w')  # create list of images for training
                ...
                for line in label_contents:
                    ...
                    if line != '':  # check if there must be an object
                        ...
                        if (len(data) == 12):
                            ...
                            line_to_write = str(kaist_names_num[class_str]) + ' ' + str(bbox_center_x) + ' ' + str(
                                bbox_center_y) + ' ' + str(bbox_width) + ' ' + str(
                                bbox_height) + '\n'  # 1 line of all texts
                            print('Writing {0} to {1}'.format(line_to_write, str(real_label)))
                            real_label.write(line_to_write)  # write to the new file
                            sys.stdout.write(
                                str(int((indexi / len(kaist_images)) * 100)) + '% ' + '*******************->' "\r")
                            sys.stdout.flush()
                            real_label.close()
